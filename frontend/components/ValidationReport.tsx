...
import { CheckCircle2, XCircle, ShieldAlert, FileText, Copy, Check, Terminal, ShieldCheck, AlertTriangle } from "lucide-react";
        ...
        {activeTab === "security" && (
          <div className="space-y-4">
            {scannersSkipped.length > 0 && (
              <div className="p-3.5 rounded-xl border border-amber-200 bg-amber-50 flex items-start gap-2.5 text-xs text-amber-900">
                <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
                <div>
                  <span className="font-bold">Not fully verified:</span>{" "}
                  {scannersSkipped.join(", ")} could not run (not installed in this
                  environment) and are not reflected in the risk score or findings below.
                </div>
              </div>
            )}
            ...
            {findings.length === 0 ? (
              scannersSkipped.length > 0 ? (
                <div className="text-center py-10 text-slate-500 bg-slate-50 rounded-xl border border-slate-200 flex flex-col items-center gap-2">
                  <AlertTriangle className="w-8 h-8 text-amber-500" />
                  <span className="text-sm font-bold text-slate-700">Not Verified, Not Failed</span>
                  <span className="text-xs text-slate-500 max-w-sm">
                    {scannersSkipped.join(", ")} did not run in this environment, so this is
                    an absence of results, not a clean scan.
                  </span>
                </div>
              ) : (
